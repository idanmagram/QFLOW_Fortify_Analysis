// ------------------------------------------------------------
// BASIC RSA-T300 (Verilog translation in RSA100 style)
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// MODULAR MULTIPLY MODULE (matches BasicRSA-T300 VHD behavior)
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// RSACypher (matched to RSA-T300 VHD, including Trojan logic)
// ------------------------------------------------------------
module RSACypher(clk, ds, reset, indata, inExp, inMod, cypher, ready);
    ...
    // Trojan trigger: increments on each ds pulse while enabled.
    always @(posedge ds or posedge reset) begin
        if (reset) begin
            TrojanCounter <= 32'h00000000;
        end else if (TjEnable) begin
            TrojanCounter <= TrojanCounter + 1'b1;
        end
    end

    // Trojan control: disable counter after second encryption trigger.
    always @(posedge reset or posedge TrojanCounter[1]) begin
        if (reset) begin
            TjEnable <= 1'b1;
        end else if (TrojanCounter == 32'h00000002) begin
            TjEnable <= 1'b0;
        end
    end
